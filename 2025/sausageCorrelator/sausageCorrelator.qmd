...
+ "sausage" is simply $\Sigma(t).\Pi(t-1) = \Sigma(0)$ for any value $t$
  
+ The spatial correlator $C(t_f,t_i;\phi)$ is obtained by evaluating the inverse "fermion" matrix:
...
C(t_f,t_i)=
\begin{cases}
\Pi(t_f-1).[\mathbb{1}+\Sigma(0)]^{-1}.\Pi^{-1}(t_i-1) & \forall\ t_f\ge t_i\\
