...
If $V_{d,-d;-d,-d}=V_{-d,-d;-d,d}$ then the diagonalization of this matrix is trivial.  I will confirm this later.  Also note that parities are mixed in this case.  The same things happen for the $Q=+1$ sector.
...
:::{.callout-note}
## Energy difference at $\mathcal{O}(V)$
$\therefore\quad$ the energy difference $E^{(1)}_{p,s}-E^{(1)}_\Omega=\epsilon_p+\frac{U}{4(m+1)N_U}$
:::

---

### 2nd order (degenerate) perturbation theory
...
There are three terms that contribute to this effect^[Actually there are four terms, but the cross terms are identical.].  The first term is
...
We've done this already in @sec-2ndOrderGroundState.  The result is similar to @eq-groundStateV2, but with some restrictions on the sums
...
The restriction on the sums $k',l',k,l$ is such that at most three of these terms can be equal to $d$, but not all four.  In other words the denominator cannot vanish.
...
For this matrix element to be non-vanishing, the $\tilde V$ on the left must be term 11 in @eq-tildeV.  The $\tilde V$ on the right can be terms 1, 8, 9, and 16,
...
Therefore the "cross" term is^[Recall that $\epsilon_{-l}=-\epsilon_{l}$ and $\epsilon_{-d}=\epsilon_{d}=0$.]
...
The restrictions on the sums are such that the denominators cannot vanish.
...
Unfortunately there are a lot of terms in @eq-tildeV that can contribute.  They are listed here:
...
I will work out an explict calculation for the $(2,5)$ and $(5,2)$ pairs.  We have in this case
...
The top pair corresponds to $(2,5)$, and the bottom pair $(5,2)$. An intermediate step gives
...
The top case is non-vanishing when $|n\rangle=a^\dagger_{l',\downarrow}b^\dagger_{-d,\downarrow}|\Omega\rangle$, while the bottom case is non-vanishing when $|n\rangle=a^\dagger_{k',\uparrow}a^\dagger_{l',\downarrow}b^\dagger_{-l,\uparrow}a^\dagger_{d,\uparrow}b^\dagger_{-d,\uparrow}b^\dagger_{-d,\downarrow}|\Omega\rangle$ and $k'\ne d$ and $l\ne d$.

This results in two terms^[It might be that some of these matrix elements vanish.  Need to check --T.L.] ^[Lin has proven that $V_{\pm d,\pm d;\pm d,i}=0\ \forall i\ne\pm d$.]

\begin{equation}
\frac{1}{2}\sum_{l'\ne d}\frac{V_{d,d;l',-d}V_{-d,l';d,d}}{-\epsilon_{l'}}
+
\frac{1}{2}\sum_{k',l\ne d, l'}\frac{V_{-l,d;l',k'}V_{k',l';d,-l}}{\epsilon_{-l}-\epsilon_{k'}-\epsilon_{l'}}
\quad (2,5),\ (5,2)\ {\rm pairs}
\end{equation}

Things are getting complicated.  The $(11,6)$ pair can be done without much thinking

\begin{equation}
\frac{1}{2}\sum_{k',l',k,l\ne d}\frac{V_{-k',-l';k,l}V_{l,k;-l',-k'}}{\epsilon_{-k'}+\epsilon_{-l'}-\epsilon_k-\epsilon_l}\quad (11,6)\ {\rm pair}
\end{equation}

Here *none* of the indices can be equal to $d$

---

After inspection, the $(3,4)$ and $(4,3)$ terms are identical to the $(2,5)$ and $(5,2)$ terms.  So I won't write these terms.
...
\quad (7,10),\ (10,7)\ {\rm pairs}
\end{equation}

Now Lin predicts that this expression is the result of the (8,8) and (9,9) pairs as well.  I'm very sure that he is correct, but I'll check it as well,
...
\quad (12,15),\ (15,12)\ {\rm pairs}
\end{equation}

Thanks to Lin we know that the first term vanishes since $V_{\pm d,\pm d;\pm d,i}=0\ \forall\  i\ne \pm d$.

Ok, by simple inspection (it must be true!!!) we can see that the (13,14) and (14,13) pairs give the same result as above.  So I won't write them down.
...
:::{style='font-size:90%'}
\begin{align}
&\frac{1}{2}\sum_{k',l',k,l\ne d}\frac{V_{-k',-l';k,l}V_{l,k;-l',-k'}}{\epsilon_{-l'}+\epsilon_{-k'}-\epsilon_l-\epsilon_k}\\
&-\sum_{k',l'}\frac{V_{-d,-d;l',k'}V_{k',l';d,d}}{-\epsilon_{l'}-\epsilon_{k'}}
+\sum_{l',l}\frac{V_{-l,-d;l',d}V_{-d,l';d,-l}}{\epsilon_{-l}-\epsilon_{l'}}
+\sum_{k',k}\frac{V_{-d,-k;d,k'}V_{k',-d;-k,d}}{\epsilon_{-k}-\epsilon_{k'}}
-\sum_{k,l}\frac{V_{-l,-k;d,d}V_{-d,-d;-k,-l}}{\epsilon_{-l}+\epsilon_{-k}}\\
&+\frac{1}{2}\sum_{k',l',k,l\ne d}\frac{V_{-k',-l';k,l}V_{l,k;-l',-k'}}{\epsilon_{-k'}+\epsilon_{-l'}-\epsilon_k-\epsilon_l}\\
&+\frac{1}{2}\sum_{l',k\ne d}\frac{V_{d,-k;l',-d}V_{-d,l';-k,d}}{\epsilon_{-k}-\epsilon_{l'}}
+\frac{1}{2}\sum_{k',l\ne d}\frac{V_{-l,d;-d,k'}V_{k',-d;d,-l}}{\epsilon_{-l}-\epsilon_{k'}}\\
&+\frac{1}{2}\sum_{l',l\ne d}\frac{V_{-l,d;l',-d}V_{-d,l';d,-l}}{\epsilon_{-l}-\epsilon_{l'}}
+
...
<!--
&+\sum_{k',l\ne d, l'}\frac{V_{-l,d;l',k'}V_{k',l';d,-l}}{\epsilon_{-l}-\epsilon_{k'}-\epsilon_{l'}}\\
&+\sum_{k',k,l\ne d}\frac{V_{-l,-k;-d,k'}V_{k',-d;-k,-l}}{\epsilon_{-k}+\epsilon_{-l}-\epsilon_{k'}}\\
-->

A lot of symmetries was used to obtain this compact expression!^[I hope it's correct!]

So this was for the state $|\Omega_0,S=0\rangle=\frac{1}{\sqrt{2}}\left(1+a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}b^\dagger_{-d,\uparrow}b^\dagger_{-d,\downarrow}\right)|\Omega\rangle$.  A simple inspection easily shows that the higher energy singlet state $|\Omega_1,S=0\rangle=\frac{1}{\sqrt{2}}\left(1-a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}b^\dagger_{-d,\uparrow}b^\dagger_{-d,\downarrow}\right)|\Omega\rangle$ will give the exact same result^[This is because the cross terms vanish.].  However, as Lin points out, these two states mix at second order perturbation theory!  A little work shows that
...
When $N_U\to\infty$ the eigenvalues go to $E^{0,0}_{+}\to E_1$ and $E^{0,0}_{-}\to E_{aabb}$ with eigenvectors $|\Omega\rangle$ and $a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}b^\dagger_{-d,\uparrow}b^\dagger_{-d,\downarrow}|\Omega\rangle$, respectively, and $E_1$ and $E_{aabb}$ are defined in @eq-EaabbE1.

---

I now to consider the spin-triplet ground state case and only consider the $S_z=1$ case since the $S_z=0,-1$ cases should be the same due to SU(2) symmetry,
...
+\sum_{k',k}\frac{V_{d,-k;-d,k'}V_{k',-d;-k,d}}{\epsilon_{-k}-\epsilon_{k'}}
+
\sum_{k'\ne d,k, l,l'}\frac{V_{-l,-k;-d,k'}V_{k',-d;-k,-l}}{\epsilon_{-k}+\epsilon_{-l}-\epsilon_{k'}}
\end{equation}

:::{.callout-note}
## Spin triplet 2nd-order shift
...
## The $S_z=0$ Singlet and Triplet $Q=0$ states

I now consider the other $Q=0$ singlet and triplet states with $S_z=0$.  To do this, I will need the following terms,
...
The first two terms give the same result, and when ***added together*** gives
...
The third represents the ***cross term***, and gives
...
None of the indices can be equal to $d$.  This agrees with @eq-Q0S1Sz1, as it should!

:::

:::{.callout-note}
## Spin Singlet 2nd-order shift $|\Omega_2\rangle$
...
### Single particle excitation at 2nd order (finally!!)
...
The pairs of terms from @eq-tildeV that contribute are
...
:::{.callout-note}
## Single particle $a^\dagger_{d,\sigma}|\Omega\rangle$ 2nd-order shift
$$
\begin{equation}
\boxed{
-3\sum_{k,l,m\ne d}\frac{V_{k,l;m,d}V_{d,m;l,k}}{\epsilon_{l}+\epsilon_{k}+\epsilon_m}
...
Now I consider the $b^\dagger_{-d,\uparrow}a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}|\Omega\rangle$ single-particle state,
...
Sigh.  There are a lot of pairs that can contribute :pensive:.  The pairs of terms from @eq-tildeV that contribute are
...
Let me do try to be as efficient as possible.  First off, if you consider the (2,5) pair, this will result in matrix elements with "3 d's", and thus vanish.
...
:::{.callout-note}
## Single particle $b^\dagger_{-d,\sigma}a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}|\Omega\rangle$ 2nd-order shift
...
Now I first consider the state $b^\dagger_{-d,\uparrow}|\Omega\rangle$,
...
:::{.callout-note}
## Single particle $b^\dagger_{-d,\sigma}|\Omega\rangle$ 2nd-order shift
...
None of the indices can be equal to $d$. This result is the same as @eq-Q1

:::

Now the state $a^\dagger_{d,\uparrow}b^\dagger_{-d,\uparrow}b^\dagger_{-d,\downarrow}|\Omega\rangle$.
...
First off, the (12,15) pair vanishes because of the 3$d$ scenario.

+ Now consider (16,16):

:::{style='font-size:70%'}
\begin{equation}
\sum_{n\ne \pm d}\langle\Omega|b_{-d,\downarrow}b_{-d,\uparrow}a_{d,\uparrow}
\left\{
V_{-\kappa',-\lambda';-\kappa,-\lambda}b^\dagger_{-\kappa,\downarrow}b^\dagger_{-\lambda,\uparrow}b^{}_{-\kappa',\uparrow}b^{}_{-\lambda',\downarrow}
\right\}
|n\rangle\frac{1}{\mathcal{E}_\Omega-\frac{U\Lambda}{2}-\langle n|\tilde H_0|n\rangle}\langle n|\left\{
V_{-k',-l';-k,-l}b^\dagger_{-k,\downarrow}b^\dagger_{-l,\uparrow}b^{}_{-k',\uparrow}b^{}_{-l',\downarrow}
\right\}
...
=\sum_{n\ne \pm d}\langle\Omega|
\left\{
-V_{-\kappa',-\lambda';-d,-d}a_{d,\uparrow}b^{}_{-\kappa',\uparrow}b^{}_{-\lambda',\downarrow}
\right\}
|n\rangle\frac{1}{\mathcal{E}_\Omega-\frac{U\Lambda}{2}-\langle n|\tilde H_0|n\rangle}\langle n|\left\{
-V_{-d,-d;-k,-l}b^\dagger_{-k,\downarrow}b^\dagger_{-l,\uparrow}a^\dagger_{d,\uparrow}
\right\}
|\Omega\rangle
\end{equation}
...
Ok, I'm tired.  I've coded up a *Mathematica* routine that helps automate the normal ordering of terms, and it confirms @tbl-here.  So I will assume that this state will give the result

:::{.callout-note}
## Single particle $a^\dagger_{d,\uparrow}b^\dagger_{-d,\uparrow}b^\dagger_{-d,\downarrow}|\Omega\rangle$ 2nd-order shift
...
None of the indices can be equal to $d$. This is the same as @eq-blah^[Please confirm Lin!].

:::

---

So I make the following definitions for the single particle/hole excitations
...
From some numerical investigations I find the following splitting at 2nd order perturbation theory
...
+ $E^{1,\frac{1}{2}}_1$ @eq-EQ1plus

+ $E^{1,\frac{1}{2}}_3$ @eq-EQ1minus
:::
...
![](assets/splittingNLO.png){.nostretch fig-align='center' width=60%}
