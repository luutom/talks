...
I assume there are degenerate single-particle states whose energy $\epsilon_d=0$.  The systems we are interested in (ie armchair ribbons), if they have a zero energy, will have a degeneracy of 4 (2 for spin, and 2 for band).  I label these single-particle degenerate states as^[The labels $a$ and $b$ for these degenerate states are completely arbitrary.]
$$
...
I define the following state $|\mathcal{V}\rangle$^[This state is ***not*** the half-filled ground state.]
$$
|\mathcal{V}\rangle=\left(\prod_{-k,s}b_{-k,s}\right)|0\rangle\quad\forall\ \epsilon_{-k}<0\ .
$$
Notice that I ***do not include*** the $b^{}_{-d,s}$ states that have degenerate zero energies.
...
Because of these single-particle degenerate states, we now have 16 *many*-body non-interacting degenerate states
$$
\begin{align}
 & &|\mathcal{V}\rangle & &  S_z=0 && Q=-2 && \Pi=+1\\
a^\dagger_{d,s}|\mathcal{V}\rangle & & b^{}_{-d,s}|\mathcal{V}\rangle & & S_z=\pm\frac{1}{2} && Q=-1 && \Pi=\pm 1\\
a^\dagger_{d,\uparrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle & & a^\dagger_{d,\downarrow}b^{}_{-d,\downarrow}|\mathcal{V}\rangle & & S_z=\pm 1 && Q=0 && \Pi=-1\\
a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}|\mathcal{V}\rangle & & b^{}_{-d,\uparrow}b^{}_{-d,\downarrow}|\mathcal{V}\rangle & & S_z=0 && Q=0 && \Pi=+1\\
a^\dagger_{d,\uparrow}b^{}_{-d,\downarrow}|\mathcal{V}\rangle & & a^\dagger_{d,\downarrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle & & S_z=0 && Q=0 && \Pi=-1\\
a^\dagger_{d,s}b^{}_{-d,\downarrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle & & a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}b^{}_{-d,s}|\mathcal{V}\rangle & & S_z=\pm\frac{1}{2} && Q=+1 && \Pi=\pm 1 \\
a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}b^{}_{-d,\downarrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle & & & &  S_z=0 && Q=2 && \Pi=+1
\end{align}
$$
All these states have eigenenergy $\mathcal{E}_\mathcal{V}-\frac{U}{2}\Lambda$, but they have different spin projection $S_z$, charge $Q$, and parity $\Pi$ as labelled above.
...
$$
\langle Q=-2|\tilde V|Q=+2\rangle=\langle \mathcal{V}|\tilde V a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}b^{}_{-d,\downarrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle
$$
A non-trivial result requires that $\tilde V$ has a term $b^\dagger_{-d,\uparrow}b^\dagger_{-d,\downarrow}a^{}_{d,\downarrow}a^{}_{d,\uparrow}$, but when one looks at the form of $\tilde V$ in @eq-tildeV, one immediately sees that there is no such term.  Thus this matrix element is zero.
:::
...
To perform degenerate perturbation theory at leading order, I need to diagonalize within the different $S_z$ subspaces.

I will consider $S_z=0$ first.  Here is the subspace:
$$
|S_z=0\rangle=
\begin{pmatrix}
 |\mathcal{V}\rangle\\
 a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}b^{}_{-d,\downarrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle\\
 a^\dagger_{d,\uparrow}a^\dagger_{d,\downarrow}|\mathcal{V}\rangle\\
 b^{}_{-d,\downarrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle\\
 a^\dagger_{d,\uparrow}b^{}_{-d,\downarrow}|\mathcal{V}\rangle\\
 a^\dagger_{d,\downarrow}b^{}_{-d,\uparrow}|\mathcal{V}\rangle 
\end{pmatrix}
$$
This gives
$$
\langle S_z=0|\tilde V|S_z=0\rangle
=
\begin{pmatrix}
 V_{-d,-d;-d,-d} & 0 & 0 & 0 & 0 & 0\\
0 &  V_{ d, d; d, d} & 0 & 0 & 0 & 0\\
0 & 0 & V_{ d, d; d, d} & V_{ d, d;-d,-d} & 0 & 0\\
0 & 0 & V_{-d,-d; d, d} & V_{-d,-d;-d,-d} & 0 & 0\\
0 & 0 & 0 & 0 & 0 & V_{-d, d;-d, d}\\
0 & 0 & 0 & 0 & V_{ d,-d; d,-d} & 0
\end{pmatrix}
$$
